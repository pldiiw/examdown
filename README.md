...
First, check if you have all the dependencies:
...
If `cmark` is missing, two little scripts are provided to help you install it.
...
From there, you can run the tests to check if everything is ok:
...
If you install to some gloomy path, be sure that the `bin` folder where
`examdown` is in your path.  
